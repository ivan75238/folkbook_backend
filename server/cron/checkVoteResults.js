...
export const checkVoteResults = () => {
    const job = new CronJob('0 */5 * * * *', function() {
        ...
        mysql.query(`SELECT \`sections\`.*, \`section_votes\`.\`id\` AS 'id_section_vote' FROM \`sections\` INNER JOIN \`section_votes\` ON \`sections\`.\`id\` = \`section_votes\`.\`id_section\` WHERE \`sections\`.\`vote_finished_at\` = '${now}'`)
            .then(result => {
