...
export const extendSection = async (mysql, section) => {
    ...
    await mysql.query(`UPDATE \`sections\` SET \`finished_at\` = '${finished_at}', \`updated_at\` = NOW(), \`vote_finished_at\` = '${vote_finished_at}' WHERE \`id\` = '${section.id}';`);
    mysql.close();
};